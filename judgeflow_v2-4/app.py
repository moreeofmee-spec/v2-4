...
def calc_score(score_obj):
    if not score_obj or not score_obj.tasks:
        return 0.0
    tasks = json.loads(score_obj.tasks)
    total = sum(float(t) for t in tasks if t not in [None, ''])
    result = (total - score_obj.error_points) * score_obj.multiplier
    return max(0.0, min(result, 30.0))  # cap at 30 (100%)

def calc_percent(val, max_pts=30):
...
def add_event(comp_id):
    ...
    event = Event(name=data['name'], competition_id=comp_id)
...
def add_judge(comp_id):
    if not session.get(f'organizer_{comp_id}'):
        return jsonify({'error': 'unauthorized'}), 403
    data = request.get_json()
    judge = Judge(name=data['name'], position=data.get('position', ''),
                  qualification=data.get('qualification', ''),
                  judge_code=data.get('judge_code', ''), competition_id=comp_id)
    db.session.add(judge)
    db.session.commit()
    return jsonify({'id': judge.id, 'name': judge.name, 'position': judge.position,
